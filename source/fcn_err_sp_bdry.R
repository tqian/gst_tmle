...
err_sp_bdry <- function(mvmean0, mvcov0, mvmean1, mvcov1, errs, v = NULL){
...
errs <- function(covmat, v, rho = 2, method = c("information", "time")){
  ...
  # rho is the tuning parameter in error spending functions,
  # method is either "information"-based, or "time"-based.
  
  if (identical(method, c("information", "time")) | identical(method, "information")){
    alpha <- v$alpha
    beta <- v$beta
    if(is.null(rho)){ rho <- v$rho }    
    f_err <- v$f_err
    g_err <- v$g_err
    K <- v$K
    ...
    fk <- sapply(Iratio, f_err, .rho = rho)
    fk_ <- sapply(c(0, Iratio[1:(K-1)]), f_err, .rho = rho)
    alphas <- fk - fk_ # as equation (12)
    
    gk <- sapply(Iratio, g_err, .rho = rho)
    gk_ <- sapply(c(0, Iratio[1:(K-1)]), g_err, .rho = rho)
    betas <- gk - gk_ # as equation (13)
  } else if (method == "time") {
    if(is.null(rho)){ rho <- v$rho }
    alphas <- sapply((1:5)/5, v$f_err, .rho = rho) - c(0, sapply((1:4)/5, v$f_err, .rho = rho))
    betas <- sapply((1:5)/5, v$g_err, .rho = rho) - c(0, sapply((1:4)/5, v$g_err, .rho = rho))
  }
...
errs_nonbinding <- function(covmat, v, rho = 2){
  # cov: an original covariance matrix (of dim 2K by 2K)
  # Note that, the vector of estimators is of length 2K, with order
  #      Z_1,\tilde{Z_1},...,Z_K,\tilde{Z_K}.
  # Output: a two row matrix, 1st row is alphas, 2nd row is betas.
...
solve_uc <- function(fcn1, fcn2, tol = 1e-05, maxiter = 200, stage,
                     print = FALSE){
  ##########
  # Solve for zero of two monotonic functions, see Notice below.
  # Return a list of zero: list(u_k = u_k, c_k = c_k).
  #
  # Keyword Arguments:
  #     fcn1    -- the first function
  #     fcn2    -- the second function
  #     tol     -- tolerance for zero
  #     maxiter -- maximum iteration for bisection method
  #     stage   -- stage of current analysis (k), for printing info
  #     print   -- if TRUE, print out each iteration
  #
  # Notice:
  #     fcn1 and fcn2 are intended to be equation (1) and (2)
  # in "2014.09.04 - Meet with Michael.lyx", with RHS moved to LHS.
  ##########
  
  u_max1 <- uniroot(fcn1, interval = c(-10,10), cc = -Inf,
                    extendInt = "yes")$root
  u_max2 <- uniroot(fcn2, interval = c(-10,10), cc = Inf,
                    extendInt = "yes")$root
  u_upper <- min(u_max1, u_max2)
  u_lower <- 0
  
  # initial value of u and c
  u_k <- (u_upper+u_lower)/2
  c_k <- uniroot(fcn1, interval = c(-10,10), uu = u_k,
                 extendInt = "yes")$root
  
  iter <- 0
  found <- 0
  while ( iter < maxiter & found == 0){
    fcn2_value <- fcn2(u_k,c_k)
    if (abs(fcn2_value) < tol) { # found!
      found <- 1
    } else if (fcn2_value > tol) { # need to increase u_k
      u_lower <- u_k
      u_k <- (u_upper+u_lower)/2
      c_k <- uniroot(fcn1, interval = c(-10,10), uu = u_k,
                     extendInt = "yes")$root
    } else if (fcn2_value < -tol) { # need to decrease u_k
      u_upper <- u_k
      u_k <- (u_upper+u_lower)/2
      c_k <- uniroot(fcn1, interval = c(-10,10), uu = u_k,
                     extendInt = "yes")$root
    ...
    iter <- iter+1
    if (print == TRUE){
      print(paste(iter, round(fcn2_value*1e05,2), round(u_k,4), round(c_k,4)))    
    }      
  }
  if (iter >= maxiter){
    stop("maxiter, no result")
  }
#   cat(sprintf("%d-th stage, iterated %d steps.\n", stage, iter))
#   cat(sprintf("\t\tfcn1 error: %f\n", fcn1(u_k, c_k)))
#   cat(sprintf("\t\tfcn2 error: %f\n", fcn2(u_k, c_k)))
  return(list(u_k = u_k, c_k = c_k))
}


err_sp_bdry_nonbinding <- function(mvmean0, mvcov0, mvmean1, mvcov1, errs, v = NULL,
                                   tol = 1e-05, maxiter = 200){
  ##########
  # Return GST thresholds, with nonbinding error spending approach.
  # Return a list of bdry: list(u = u, l = l, c = c).
  #
  # Keyword Arguments:
  #     mvmean0 -- mean vector of standardized test statistics, under H0
  #     mvcov0  -- covariance matrix of standardized test statistics, under H0
  #     mvmean1 -- mean vector of standardized test statistics, under H1
  #     mvcov1  -- covariance matrix of standardized test statistics, under H1
  #     errs    -- matrix of 2 rows: 1st row the alphas, 2nd row the betas at each stage
  #     v       -- list of trial setting, default = NULL
  #     tol     -- tolerance for zero, pass into solve_uc()
  #     maxiter -- maximum iteration for bisection method, pass into solve_uc()
  #
  # Notice:
  #     The vector of estimators is of length 2K, with order Z_1,\tilde{Z_1},...,Z_K,\tilde{Z_K}.
  #     mvmean0, mvcov0, mvmean1, mvcov1 can be obtained using compcov().
  #     errs can be obtained using errs_nonbinding().
  #     v is here for flexibility. Not used in current code.
  #     Boundaries computed according to equation (1) and (2) in "2014.09.04 - Meet with Michael.lyx".
  ##########
  ...
  # The boundaries: u[k] (efficacy), l[k] (futility), c[k] (decision analysis)
  u <- c <- as.numeric(rep(NA, K))
  l <- rep(-Inf, K) # nonbinding
  ...
  for(k in 1:K){
    ...
    if (k == 1){
      ...
      fcn1 <- function(uu, cc){
        pmvnorm(lower = c(uu, cc), upper = c(Inf, Inf),
                mean = mvmean0[1:2], sigma = mvcov0[1:2, 1:2]) - alphas[1]}
      fcn2 <- function(uu, cc){
        pmvnorm(lower = c(uu, -Inf), upper = c(Inf, cc),
                mean = mvmean1[1:2], sigma = mvcov1[1:2, 1:2]) - betas[1]}      
      uc <- solve_uc(fcn1, fcn2, tol = tol, maxiter = maxiter, stage = k)
      u[1] <- uc$u_k
      c[1] <- uc$c_k
      
    } else if (k < K){
      
      idx <- c(seq(from = 1, by = 2, length.out = k), 2*k)
      fcn1 <- function(uu, cc){
        pmvnorm(lower = c(rep(-Inf,k-1), uu, cc), upper = c(u[1:(k-1)], Inf, Inf),
                mean = mvmean0[idx], sigma = mvcov0[idx, idx]) - alphas[k]}
      fcn2 <- function(uu, cc){
        pmvnorm(lower = c(rep(-Inf,k-1), uu, -Inf), upper = c(u[1:(k-1)], Inf, cc),
                mean = mvmean1[idx], sigma = mvcov1[idx, idx]) - betas[k]}      
      uc <- solve_uc(fcn1, fcn2, tol = tol, maxiter = maxiter, stage = k)      
      u[k] <- uc$u_k
      c[k] <- uc$c_k
      ...
      idxc <- c(seq(from = 1, by = 2, length.out = (K-1)), 2*K)
      c[K] <- uniroot( # equation (15) of Hampson & Jennison 2013 JRSSB
        function(x){
