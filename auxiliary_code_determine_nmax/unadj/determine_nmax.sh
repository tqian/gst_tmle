...
cd MISTIE/code_determine_nmax # set working directory
...
qsub -t 1:50 determine_nmax_parallel.sh # submit parallel jobs

while [ ! -e "$FILE_LOOP_DONE" ];
do
sleep 10 # wait until parallel jobs are finished
done
