...
setwd("MISTIE/code_determine_nmax/exL")

source("fcn_comppower.R")

nameH0 <- "Result/nmax_H0exL_enrollrate140"
nameH1 <- "Result/nmax_H1exL_enrollrate140"
...
while ( finished == 0 ){ # terminating flag
  ...
  if (abs(right_nmax - left_nmax) < error_nmax | abs(power - 0.8) < error_power) { # found!
    write.table(paste0("enrollrate = 140, exL, nmax is found to be ", nmax), file = "nmax_found.txt",
