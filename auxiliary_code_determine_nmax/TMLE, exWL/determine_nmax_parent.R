...
setwd("MISTIE/code_determine_nmax/exWL")

source("fcn_comppower.R")

nameH0 <- "Result/nmax_H0exWL_enrollrate140"
nameH1 <- "Result/nmax_H1exWL_enrollrate140"
...
while ( finished == 0 ){ # terminating flag
  ...
  power <- comppower(v, caseH0, caseH1, 1:Npara, nonbinding = FALSE)[2]
  ...
  if (abs(right_nmax - left_nmax) < error_nmax | abs(power - 0.8) < error_power) { # found!
    write.table(paste0("enrollrate = 140, exWL, nmax is found to be ", nmax), file = "nmax_found.txt",
